...
docker build --build-arg GRADLE_VERSION=${_gradle_version} --tag "stakater/gradle-alpine:${_gradle_tag}"  --no-cache=true .

if [ $_release_build == true ]; then
	docker tag "stakater/gradle-alpine:${_gradle_tag}" "stakater/gradle-alpine:latest"
fi